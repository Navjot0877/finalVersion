...
// Fetching Product Info
    useEffect(()=> {
            Axios.get("https://luminous-liquor.herokuapp.com/productInfo/" + id.id).then((response)=>{
...
// Reading if the product is in cart or not
    useEffect(()=> {
        Axios.get("https://luminous-liquor.herokuapp.com/readCart/"+id.id+ '/' + userId).then((response)=>{
    ...
    useEffect(()=> {
      Axios.get("https://luminous-liquor.herokuapp.com/readFav/"+id.id+ '/' + userId).then((response)=>{
  ...
  console.log(total)
    ...
    const handleAdd = () =>{
       if (added){
        Axios.post("https://luminous-liquor.herokuapp.com/delFav/"+id.id+'/'+userId)
        setOpenCart(false);
           setAdd(false);
           setremoveFav(true)
           setOpenFav(false)
        ...
        setOpenCart(false);
          ...
          //  alert(userId)
           ...
           Axios.post("https://luminous-liquor.herokuapp.com/favourite/"+val.prodId+'/'+ userId +'/'+val.name+'/'+val.description+'/'+val.price),
...
if(addedCart==true){
    ...
    Axios.post("https://luminous-liquor.herokuapp.com/delCart/"+id.id+'/'+userId)
    setAddLabel("Add to Cart")
}
else{
...
setAddCart(true);
    ...
    Axios.post("https://luminous-liquor.herokuapp.com/cart/"+val.prodId+'/'+ userId +'/'+val.name+'/'+val.description+'/'+val.price)
  );
...
//     }
  ...
  useEffect(()=> {
    // alert("enetered")
    Axios.get("https://luminous-liquor.herokuapp.com/productprice/" + id.id).then((response)=>{
      // alert("enetered")
    ...
    const toComponentB=()=>{
      ...
      navigate('/checkout',{state:{id:id.id,quantity:counter,price:price}});
        ...
        return(
          ...
          {productInfoList.slice(0, 8).map((pm) => (
              <Zoom className="zoo">
              <img className="picture_pic" src={pm.image} alt=""></img>
              </Zoom>
