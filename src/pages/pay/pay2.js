...
// Declaring UseStates
  ...
  const [newTotal, setNewTotal] = useState(location.state.total2);
...
// const [newTax, setTax] = useState(newTotal*0.05)
// const [newGrand, setGrand] = useState(newTotal + ((newTotal)*0.05) +5)

var alltotal = (parseFloat(location.state.total)).toFixed(2);
var taxi =  (parseFloat(alltotal * 0.05)).toFixed(2);
var grandi = (parseFloat(alltotal) + parseFloat(taxi) + 5).toFixed(2);
...
//     }
   ...
   async function process(){
    ...
    Axios.post("https://luminous-liquor.herokuapp.com/sendme/"+ userName+'/'+pay+'/' +taxi+ '/'+ address+'/'+readPost+ '/'+myEmail+ '/' + alltotal).then((res) => {
