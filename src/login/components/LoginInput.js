...
const LoginInput = ({ setLogInUser }) => {
  ...
  useEffect(() => {
   ...
   function handleCallback(response){
    ...
    try {
      const res = axios.post("https://luminous-liquor.herokuapp.com/googleLogin", {tokenId: response.credential}).then((res) => {
      ...
      axios.put('https://luminous-liquor.herokuapp.com/api/convo/', convo)
    ...
    try {
      ...
      axios.post("https://luminous-liquor.herokuapp.com/facebookLogin", {accessToken, userID}).then((res) => {
        if(res.data.message != "Login Success"){
          setOpen3(true);
          }
  ...
  const login = () => {
    axios.post("https://luminous-liquor.herokuapp.com/login", user).then((res) => {
      ...
      setLogInUser(res.data.user);
      if(res.data.user.email === "projectcapstone007@gmail.com"){
        sessionStorage.setItem('myUser', JSON.stringify(res.data.user))
        let x = sessionStorage.getItem('myUser')
        x = JSON.parse(x)
        console.log(x.email)
        window.location.replace('http://localhost:3000?email=' + x.email + '&key=' + x.password)
      }
      else if(res.data.user.email === user.email){
        const convo = {
          senderId: res.data.user._id,
          receiverId: '62d1de891f770f0cd301ac00',
        }
...
<div className="needyou">
      <label>
        Need an account? Please <Button className="butReg" onClick={(()=> chalRegister())}>Register</Button>
      </label>
      </div>
